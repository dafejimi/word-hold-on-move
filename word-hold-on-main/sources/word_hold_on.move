...
module overmind::word_hold_on {
    ...
    //use friend aptos_framework::create_signer::{create_signer};
    #[test_only]
    ...
    */
    fun init_module(overmind: &signer) {
        ...
        let (resource_account, resource_account_cap) = account::create_resource_account(overmind, SEED);
        ...
        check_if_account_has_enough_aptos_coins(resource_account_address, PRIZE);
        // TODO: Transfer `PRIZE` amount of APT from `overmind` to the resource account
        coin::transfer<AptosCoin>(overmind, resource_account_address, PRIZE);
    ...
    fun create_letter_array(): vector<vector<u8>> {
        let letters_vector: vector<vector<u8>> = vector::empty<vector<u8>>();
        vector::push_back(&mut letters_vector, FIRST_LETTER_HASH);
        vector::push_back(&mut letters_vector, SECOND_LETTER_HASH);
        vector::push_back(&mut letters_vector, THIRD_LETTER_HASH);
        vector::push_back(&mut letters_vector, FOURTH_LETTER_HASH);
        vector::push_back(&mut letters_vector, FIFTH_LETTER_HASH);
        vector::push_back(&mut letters_vector, SIXTH_LETTER_HASH);
        vector::push_back(&mut letters_vector, SEVENTH_LETTER_HASH);
        vector::push_back(&mut letters_vector, EIGHTH_LETTER_HASH);
        letters_vector
    }
    ...
    */
    public entry fun guess_word(player: &signer, word: String) acquires State, Player {
        ...
        let player_resource: &mut Player = borrow_global_mut<Player>(player_address);
        ...
        let word_bytes: &vector<u8> = string::bytes(&word);
        let word_hash: vector<u8> = hash::sha3_256(*word_bytes);
        ...
        let player_attempt: SimpleMap<u8, SimpleMap<vector<u8>, bool>> = player_resource.attempts;
        let attempt_count: u64 = simple_map::length<u8, SimpleMap<vector<u8>, bool>>(&player_attempt);
    ...
    */
    ...
    public fun get_guess_attempts(player_address: address): SimpleMap<u8, SimpleMap<vector<u8>, bool>> acquires Player {
        ...
        if (exists<Player>(player_address)) {
            let player_resource: &Player = borrow_global_mut<Player>(player_address);
            player_resource.attempts
        } else {
            let simple_map: SimpleMap<u8, SimpleMap<vector<u8>, bool>> = simple_map::new<u8, SimpleMap<vector<u8>, bool>>();
            simple_map
        }
    }
    ...
    */
    inline fun submit_correct_answer(player_resource: &mut Player, player_address: address) {
        ...
        let overmind: signer;
        coin::transfer<AptosCoin>(&overmind, player_address, PRIZE);
        ...
        let submit_correct_answer_event: SubmitCorrectAnswerEvent = SubmitCorrectAnswerEvent {
            attempt_count,
            timestamp::now_seconds();
        };
        event::emit_event<SubmitCorrectAnswerEvent>(
            &mut player_resource.submit_correct_answer_events,
            submit_correct_answer_event,
        );
    }
    ...
    */
    inline fun add_guess_to_attempts(player_resource: &mut Player, player_address: &address, word: String) {
        ...
        let results_simple_map: SimpleMap<vector<u8>, bool> = simple_map::new<vector<u8>, bool>();
        // TODO: Get current timestamp
        let attempt_timestamp: u64 = timestamp::now_seconds();
        ...
        let word_bytes_ref: &vector<u8> = string::bytes(&word);
        let word_bytes: vector<u8> = *word_bytes_ref;
        let result: bool;
        let i: u64= 0;
        loop {
            if (i < 8) {
                let letter_vector: vector<u8> = vector::empty<u8>();
                let letter_hash_at_index: vector<u8> = get_letter_hash(i);
                let letter_ref: &u8 = vector::borrow<u8>(&mut word_bytes, i);
                let letter: u8 = *letter_ref;
                vector::push_back(&mut letter_vector, letter);
                if (letter_hash_at_index == letter_vector){
                    result = true;
                } else {
                    result =false
                };

                let i_vector: vector<u8> = bcs::to_bytes(&i);
                let timestamp_vector: vector<u8> = bcs::to_bytes(&attempt_timestamp);
                let address_vector: vector<u8> = bcs::to_bytes(player_address);

                let composite_hash: vector<u8> = get_composite_hash(letter_vector, i_vector, timestamp_vector, address_vector);
                simple_map::add<vector<u8>, bool>(&mut results_simple_map, composite_hash, result); 
        ...
        let player_resource_attempts: SimpleMap<u8, SimpleMap<vector<u8>, bool>> = player_resource.attempts;
        let current_attempt: u64 = simple_map::length<u8, SimpleMap<vector<u8>, bool>>(&player_resource_attempts);
        ...
        simple_map::add<u8, SimpleMap<vector<u8>, bool>>(&mut player_resource_attempts, (current_attempt as u8), results_simple_map);

        // TODO: Emit `GuessWordAttemptEvent` event
        let guess_word_attempt_event: GuessWordAttemptEvent = GuessWordAttemptEvent {
            (current_attempt as u8),
            word,
            results_simple_map,
            timestamp::now_seconds(),
        };
        event::emit_event<GuessWordAttemptEvent>(
            &mut player_resource.guess_word_attempt_events,
            guess_word_aattempt_event,
        );
    ...
    */
    inline fun get_letter_hash(index: u64): vector<u8> {
        ...
        assert!(index >= 0 && index <=7, ELetterIndexOutOfBounds);
        let letters_vector: vector<vector<u8>> = create_letter_array();
        let letter_hash: &vector<u8> = vector::borrow<vector<u8>>(&mut letters_vector, index);
        *letter_hash
    }

    fun get_composite_hash(vector_1: vector<u8>, vector_2: vector<u8>, vector_3: vector<u8>, vector_4: vector<u8>): vector<u8> {
        let composite_vector = vector::empty<vector<u8>>();
        let vector_1_hash: vector<u8> = hash::sha3_256(vector_1);
        vector::push_back(&mut composite_vector, vector_1_hash);

        let vector_2_hash: vector<u8> = hash::sha3_256(vector_2);
        vector::push_back(&mut composite_vector, vector_2_hash);

        let vector_3_hash: vector<u8> = hash::sha3_256(vector_3);
        vector::push_back(&mut composite_vector, vector_3_hash);

        let vector_4_hash: vector<u8> = hash::sha3_256(vector_4);
        vector::push_back(&mut composite_vector, vector_4_hash);

        let composite_hash: vector<u8> = bcs::to_bytes(&composite_vector);
        composite_hash
    }
    ...
    inline fun check_if_account_has_enough_aptos_coins(account: address, apt_amount: u64) {
        ...
        let balance: u64 = coin::balance<AptosCoin>(account);
    ...
    inline fun check_if_player_has_not_reached_attempts_limit_yet(player_resource: &Player) {
        ...
        let player_attempt: SimpleMap<u8, SimpleMap<vector<u8>, bool>> = player_resource.attempts;
        let attempt_count: u64 = simple_map::length<u8, SimpleMap<vector<u8>, bool>>(&player_attempt);
        assert!(attempt_count < ATTEMPTS_LIMIT, EAttemptsLimitReached);
    }
    ...
    inline fun check_if_word_length_is_correct(word: &String) {
        ...
        let word_length: u64 = string::length(word);
        assert!(word_length == 8u64, EInccorectWordLength);
    }
